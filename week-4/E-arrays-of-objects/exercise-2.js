...
*/
travelDestinations = [destination1, destination2, destination3, destination4];
var destinationNamesWithin500Kms = travelDestinations
  .filter(dest => dest.distanceKms <= 500)
  .map(milley => milley.destinationName); // Complete here

var destinationNameReachableByFerry = travelDestinations
  .filter(jj => jj.transportations.includes("ferry"))
  .map(name => name.destinationName);
var destinationNamesMoreThan300KmsAwayByTrain = travelDestinations
  .filter(jj => jj.distanceKms > 300 && jj.transportations.includes("train"))
  .map(names => names["destinationName"]); // Complete here // Complete here (PRINT THE RESULT IN THE CONSOLE USING FOREACH)
/*
